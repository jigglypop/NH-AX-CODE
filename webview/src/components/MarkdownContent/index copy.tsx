...
const createReferenceCitationItems = (
  ...
  const items = Array.from(numberGroup.matchAll(/\d+/g), (match): ReferenceCitationItem => {
    ...
    const target = getReferenceLinkUrl(referenceLinks[referenceNumber]);
...
const renderReferenceCode = (
  ...
  const citationItems = createReferenceCitationItems(match[1], match[2], referenceLinks);
...
const createStreamdownComponents = (referenceLinks?: Record<number, ReferenceLinkTarget>): Components => ({
  inlineCode: ({ children, className, node: _node, ...props }) => {
    void _node;

    const referenceCode = renderReferenceCode(children, referenceLinks);
...
export const MarkdownContent = ({
  ...
  const streamdownComponents = createStreamdownComponents(referenceLinks);
  ...
  const activateReferenceLink = (
    ...
    const sourceUrl = Number.isFinite(referenceNumber)
      ? getReferenceLinkUrl(referenceLinks?.[referenceNumber])
      : undefined;
